...
import 'package:fab/models/addCoaching.dart';
...
Future<List<Map<String, dynamic>>> getHabits() async {
  ...
  Future<void> removeHabit(String id,String userEmail) async {
    await _firestore
        .collection('testers').doc(userEmail).collection('habits')
...
Future<void> updateHabitStatus(bool iscompleted, String id,String userEmail) async {
...
Future<void> addCoaching(
  ...
  CoachingDetail newTask = CoachingDetail.fromMap(document.data() as Map<String, dynamic>);
...
// }
  ...
  if (querySnapshot.docs.isNotEmpty) {
    ...
    Timestamp firestoreTimestamp = Timestamp.fromDate(DateTime.parse(timestamp));
...
Future<void> deleteTaskNote({
    ...
    QuerySnapshot querySnapshot = await tasksCollection
        .where('objectID', isEqualTo: taskID)
        .get();
